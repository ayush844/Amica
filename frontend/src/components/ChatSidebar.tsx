    ...
    <aside className={` fixed sm:static top-0 left-0 h-screen w-80 bg-gray-900 border-r border-gray-700 transform ${sidebarOpen?"translate-x-0" : "-translate-x-full"} sm:translate-x-0 transition-transform duration-300 flex flex-col z-40 sm:static sm:z-auto`}>
        ...
        <div className="p-6 border-b border-gray-700">
            ...
            <div className="flex items-center justify-between">
                ...
                <button className={` p-2.5 rounded-lg transition-colors cursor-pointer ${showAllUsers?" bg-red-600 hover:bg-red-700 text-white": " bg-green-600 hover:bg-green-700 text-white"}`} onClick={()=>setShowAllUsers((prev) => !prev)}>
                    {showAllUsers ? (
                        <X className=' w-4 h-4' />
                    ) : (
        ...
        <div className=' flex-1 overflow-hidden px-4 py-2'>
            {
                showAllUsers ? (<div className=' space-y-4 h-full'>
                    <div className="relative">
                        ...
                        <input type='text' placeholder='Search Users...' className=' w-full pl-10 pr-4 py-3 bg-gray-800 border border-gray-700 text-white placeholder-gray-400' value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} />
                    </div>

                    {/* users list */}

                    <div className=" space-y-2 overflow-y-auto h-full pb-4">
                        {users?.filter((u) => u._id !== loggedInUser?._id && u.name.toLowerCase().includes(searchQuery.toLowerCase())).map((u) => (
                            <button key={u._id} className=' w-full text-left p-4 rounded-lg border border-gray-700 hover:border-graay-600 hover:bg-gray-800 transition-colors' onClick={()=>createChat(u)}>
                                <div className="flex items-center gap-3">
                                    <div className="relative">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className=" flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                {
                                                    unseenCount > 0 && <div className=" bg-red-600 text-xs text-white font-bold rounded-full min-w-[22px] h-5.5 flex items-center justify-center px-2">
